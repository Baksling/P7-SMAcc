...
declaration number_parser(const string& line)
{
    ...
    return declaration(double_type, take_while(line_wo_ws, '='), parse_equation2(nums), 1);
}
...
list<declaration> declaration_parser::parse_clocks(const string& line)
{
    ...
    while(std::getline(clocks_stream, clock, ','))
    {
        ...
        if (clock_without.find('=') != std::string::npos)
            result.emplace_back(clock_type, take_while(clock_without, '='), get_expr_value_string(clock_without),global_clock_id_counter_++);
        else
