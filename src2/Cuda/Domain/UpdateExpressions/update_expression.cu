...
void update_expression::evaluate(cuda_stack<double>* stack, const lend_array<clock_timer_t>* timers,
                                   const lend_array<system_variable>* variables) const
{
    ...
    case division_e:
        ...
        break;
    case power_e:
        if(stack->count() < 2) printf("stack not big enough to evaluate power expression\n");
        v2 = stack->pop();
        v1 = stack->pop();
        stack->push( pow(v1, v2) );
        break;
    case sqrt_e:
        if(stack->count() < 1) printf("stack not big enough to evaluate sqrt expression\n");
        v1 = stack->pop();
        stack->push( sqrt(v1) );
        break;
    case negation_e:
        if(stack->count() < 1) printf("stack not big enough to evaluate negation expression\n");
        v1 = stack->pop();
        stack->push( -v1 );
        break;
    case conditional_e:
        if(stack->count() < 2) printf("stack not big enough to evaluate negation expression\n");
        v2 = stack->pop();
        stack->pop();
        stack->push(v2);
        break;
    case less_equal_e:
        if(stack->count() < 2) printf("stack not big enough to evaluate less_equal expression\n");
        v2 = stack->pop();
        v1 = stack->pop();
        stack->push( v1 <= v2 );
        break;
    case greater_equal_e:
        if(stack->count() < 2) printf("stack not big enough to evaluate greater_equal expression\n");
        v2 = stack->pop();
        v1 = stack->pop();
        stack->push( v1 >= v2 );
        break;
    case less_e:
        if(stack->count() < 2) printf("stack not big enough to evaluate less expression\n");
        v2 = stack->pop();
        v1 = stack->pop();
        stack->push( v1 < v2 );
        break;
    case greater_e:
        if(stack->count() < 2) printf("stack not big enough to evaluate greater expression\n");
        v2 = stack->pop();
        v1 = stack->pop();
        stack->push( v1 > v2 );
        break;
    case equal_e:
        if(stack->count() < 2) printf("stack not big enough to evaluate equal expression\n");
        v2 = stack->pop();
        v1 = stack->pop();
        stack->push( v1 == v2 );  // NOLINT(clang-diagnostic-float-equal)
        break;
    case not_equal_e:
        if(stack->count() < 2) printf("stack not big enough to evaluate not_equal expression\n");
        v2 = stack->pop();
        v1 = stack->pop();
        stack->push( v1 != v2 );  // NOLINT(clang-diagnostic-float-equal)
        break;
    case not_e:
        if(stack->count() < 1) printf("stack not big enough to evaluate not expression\n");
        v1 = stack->pop();
        stack->push( v1 == 0 ? 1.0 : 0.0   );  // NOLINT(clang-diagnostic-float-equal)
        break;
    // default:
