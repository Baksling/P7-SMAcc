...
void result_writer::write_results(const simulation_result* sim_result, const unsigned result_size,
    const unsigned variable_count, steady_clock::duration sim_duration) const
{
    const std::string filepath = this->path_ + this->base_filename_ + std::to_string(file_count_) + ".txt";
    std::ofstream file;
    file.open(filepath);
    ...
    {
        const variable_result* result = var_result.at(i);
